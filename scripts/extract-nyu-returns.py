#!/usr/bin/env python3
"""
Extract NYU Stern historical return data and generate TypeScript file.

Input:  src/lib/calc/historical-data/nyu-returns.csv
Output: src/lib/calc/historical-data/nyu-returns.ts

Parses the NYU Stern CSV (percentage strings) into decimal annual returns:
...
INPUT_CSV = os.path.join(project_root, "src/lib/calc/historical-data/nyu-returns.csv")
OUTPUT_TS = os.path.join(project_root, "src/lib/calc/historical-data/nyu-returns.ts")
 ...
 *
 * Contents (all values as decimals, e.g. 0.12 = 12%):
 *   stockReturn   — Real annual S&P 500 return (including dividends)
 *   bondReturn    — Real annual 10-year U.S. Treasury Bond return
 *   cashReturn    — Real annual 3-month U.S. Treasury Bill return
 *   inflationRate — Annual CPI inflation rate
 *
 * Generated by: scripts/extract-nyu-returns.py from nyu-returns.csv
 * Used by: LcgHistoricalBacktestReturnsProvider for historical backtesting
 */
