#!/usr/bin/env python3
"""
Extract Shiller yield data and generate TypeScript file.

Input:  src/lib/calc/historical-data/shiller-yields.csv
Output: src/lib/calc/historical-data/shiller-yields.ts

Parses Robert Shiller's dataset to extract December-only observations of:
- Stock dividend yield (S&P Composite: 12 x monthly dividend / price)
- Bond yield (10-year Treasury GS10)

Data spans 1928-present. Prints summary statistics after generation.
"""
...
INPUT_CSV = os.path.join(project_root, "src/lib/calc/historical-data/shiller-yields.csv")
OUTPUT_TS = os.path.join(project_root, "src/lib/calc/historical-data/shiller-yields.ts")
 ...
 *
 * Contents (all values as decimals, e.g. 0.045 = 4.5%):
 *   stockYield — Annualized S&P Composite dividend yield: (12 x monthly dividend) / price
 *   bondYield  — 10-year U.S. Treasury bond yield (GS10)
 *
 * Only December observations are included (one value per year).
 *
 * Generated by: scripts/extract-shiller-yields.py from shiller-yields.csv
 * Used by: LcgHistoricalBacktestReturnsProvider for historical yield data in backtesting
 */
